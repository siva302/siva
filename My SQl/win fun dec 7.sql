...
SELECT
    Product,Year
    Sale,
    first_VALUE(Product) OVER ( ORDER BY Year) least_product
FROM
    Sales;
select * from sales order by year;
#lastvalue()--
...
-- > row no
 select *,
row_number() over (order by year)  order1 from sales;
-- > desc
select *,
row_number() over(order by year DESC) from sales; 
-- > rank
select*, rank() over(order by year) as new_year from sales;
-- > dense rank
select*,dense_rank() over(order by year) as new_year from sales;
